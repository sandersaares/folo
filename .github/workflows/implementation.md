...
A trusted controller build of `cargo-release-plan` independently prepares offline resolution,
regenerates the proposal, and previews it to a fixed point in a new owned reference worktree.
It exports the final version-target set with structural `expand`, compares that portable
artifact with the worker's recorded export, and applies the full captured state without publication.
The export omits machine-local snapshot/evidence paths; the resolved artifact is never silently
projected or used as a portable comparison document. Its original-root binding still controls apply.
...
The recorded artifact digest must match the complete portable export. Unsupported schemas require
fresh evidence; no compatibility path performs a late dependency refresh.
