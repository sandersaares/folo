...
# caught by a test rather than by a wasted CI mutation run. The recipe owns environment preparation
# and command construction; this module also launches its final literal native argument array.
#
...
function Get-MutantsExcludeArgument {
    ...
    # the current platform. Callers pass these literal values as an argument array, so wildcard
    # syntax belongs to cargo-mutants rather than a shell.
    ...
    $exclude = @(
        ...
        '-e', 'packages/many_cpus_impl/src/pal/linux/filesystem/**',
        ...
        '-e', 'packages/cargo-bench-history-faker/**',
        ...
        '-e', 'packages/cargo-bench-history-figures/**',
        ...
        '-e', 'packages/cbh_detect/src/testing.rs',
        ...
        '-e', 'packages/cbh_detect/src/detect/examples.rs',
        '-e', 'packages/cbh_detect/src/detect/scatter.rs',
        ...
        '-e', 'packages/par_bench/src/resource_usage_ext.rs',
        ...
        '-e', 'packages/dure/src/pal/**/windows.rs',
        '-e', 'packages/dure/src/pal/**/memory.rs',
        '-e', 'packages/dure/src/pal/raw_handle.rs',
        ...
        '-e', 'packages/dure/src/outbox.rs',

        # The integration-test helper package and in-crate test support are not product code.
        '-e', 'packages/dure-test-helper/**/*.rs',
        '-e', 'packages/dure/src/test_support.rs'
    )
...
function Invoke-MutantsCommand {
    # `just mutants` owns argument construction. This native boundary avoids PowerShell's
    # Unix wildcard expansion, retaining literal exclusions without quote characters in argv.
    [CmdletBinding()]
    [OutputType([int])]
    ...
    try {
        if (-not $process.Start()) { throw 'Could not start cargo-mutants.' }
        $process.WaitForExit()
        return $process.ExitCode
    } finally { $process.Dispose() }
}

Export-ModuleMember -Function Get-MutantsExcludeArgument, Get-MutantsShardArgument, Invoke-MutantsCommand
